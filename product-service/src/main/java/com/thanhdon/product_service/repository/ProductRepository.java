...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("select distinct p from Product p join ProductVariant v on p.id = v.product.id where (:size is null or v.size in :size) and (:color is null or v.color in :color)")
    List<Product> findProductByVariantAttributes(@Param("color") List<String> size,@Param("size") List<String> color);
//
